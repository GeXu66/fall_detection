    ...
    try:
        ...
        while True:
            ...
            if bed_mask is not None:
                ...
                # Draw B point: center of the bed mask bounding box (with manual offset, clamped to frame)
            ...
            # (Removed legacy overlap/keypoint mask heuristics; using center-distance rule instead)
            ...
            for i, box in enumerate(boxes):
                ...
                # Draw A point: center of the person bbox
                ax = int((x1i + x2i) * 0.5)
                ay = int((y1i + y2i) * 0.5)
                ...
                if falling and bed_bbox is not None:
                    ...
                    thresh_px = lie_threshold * float(width)
                    on_bed = dist_px <= thresh_px
                    if on_bed:
                        status_text = "On Bed"
                        color = (255, 0, 0)
                    else:
                    ...
                    if log_every > 0 and (frame_index % max(1, log_every) == 0):
                        logger.debug(
                            f"frame={frame_index} track={tid} bed_check(center-dist): dist_px={dist_px:.1f} <= thresh_px={thresh_px:.1f} -> {status_text}"
                        )
                else:
